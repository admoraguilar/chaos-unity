...
public class ScifiFXUI : ShaderGUI
{
	...
	void DoMain() 
	{
        ...
        MaterialProperty _Tilin_X = FindProperty("_Tilin_X");
        editor.ShaderProperty(_Tilin_X, MakeLabel(_Tilin_X));


        MaterialProperty _Tilin_Y = FindProperty("_Tilin_Y");
        editor.ShaderProperty(_Tilin_Y, MakeLabel(_Tilin_Y));
        ...
        //LABELS
        GUILayout.Label("Custom Texture", EditorStyles.helpBox);

        MaterialProperty _MatcapTexture = FindProperty("_MatcapTexture");
        editor.ShaderProperty(_MatcapTexture, MakeLabel(_MatcapTexture));

        MaterialProperty _CustomTexture = FindProperty("_CustomTexture");
        editor.ShaderProperty(_CustomTexture, MakeLabel(_CustomTexture));

        MaterialProperty _NormalMap = FindProperty("_NormalMap");
        editor.ShaderProperty(_NormalMap, MakeLabel(_NormalMap));
    }
}