 ...
 * Copyright(c) 2015-2021, Jorge Pinal Negrete.  All Rights Reserved. 
 *  
*/

Shader "PIDI Shaders Collection/Planar Reflections 4/Water Shaders/Mobile Water (Depth)" {
	Properties {
		
		[Header(Water Settings)][Space(8)]
		_WaterClarity( "Water Clarity", Range(0,1)) = 0.5
		_WaterBottomLevel( "Water Bottom Level", Range(1,10)) = 0
		_FinalTint( "Final Tint", Color ) = (0,0.6,0.85,1)		
		...
		[HideInInspector][PerRendererData]_ReflectionTex("Reflection Tex", 2D) = "black"{}
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandardSpecular o) {
...
#if _USE_FOG
			fixed4 fogColor = tex2D(_ReflectionFog, reflectionUV);
			refColor = lerp(refColor, fogColor, fogColor.a);
#endif
			...
			o.Emission =  lerp(refColor*_FinalTint, refraction *c.rgb,pow(dot(normalize(IN.viewDir),o.Normal),1.5));//;//;
