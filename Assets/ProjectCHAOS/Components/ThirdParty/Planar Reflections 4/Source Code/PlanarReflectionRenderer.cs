    ...
    */
    ...
    public class ReflectionData {
        ...
        public void RegenerateTextures( PlanarReflectionSettings settings ) {
            ...
            rd.msaaSamples = settings.useAntialiasing ? 4 : 1;
    ...
    public class PlanarReflectionRenderer : MonoBehaviour {
        ...
        public Material worldFogMaterial;
        ...
        public string Version { get { return "4.0.7"; } }
        ...
        protected List<Camera> _reflectionCameras = new List<Camera>();
        ...
        public void OnEnable() {
            ...
            if ( !worldFogMaterial ) {
                var mats = UnityEditor.AssetDatabase.FindAssets( "t:material PIDIPlanarReflections4_WorldBasedFog" );
                if ( mats.Length > 0 ) {
                    worldFogMaterial = UnityEditor.AssetDatabase.LoadAssetAtPath<Material>( UnityEditor.AssetDatabase.GUIDToAssetPath( mats[0] ) );
                }
            }
            ...
            Camera.onPreCull -= RenderLegacyReflection;
            Camera.onPreCull += RenderLegacyReflection;
        ...
        public void RenderLegacyReflection( Camera cam ) {


            if ( !_reflectionData.ContainsKey( cam )
#if UNITY_EDITOR
                || _sceneReflection == null || !_sceneReflection.ReflectionCamera
#endif
                ) {

                if ( _reflectionCameras.Count < 1 ) {
                    var rCam = new GameObject( "_ReflectionCamera", typeof( Camera ), typeof( Skybox ) );
...
#endif
                if ( cam.cameraType == CameraType.Game && !_reflectionData.ContainsKey( cam ) && cam.gameObject.hideFlags == HideFlags.None ) {
            ...
            refCamera.backgroundColor = _settings.clearToColor ? _settings.backgroundColor : cam.backgroundColor;
            ...
            if ( refCamera.TryGetComponent<PostProcessLayer>( out postFX ) ) {
                ...
                postFX.stopNaNPropagation = false;
                postFX.fog.enabled = !_settings.renderFog;
            }
            ...
            var tempShadows = QualitySettings.shadows;

            QualitySettings.lodBias *= _settings.customLODBias;
            QualitySettings.maximumLODLevel = _settings.maxLODLevel;
            QualitySettings.shadows = _settings.renderShadows ? tempShadows : ShadowQuality.Disable;
            ...
            ( Time.realtimeSinceStartup > currentData.timer ) ) {
                ...
                if ( _settings.renderFog ) {
                    ...
                    postFX.enabled = false;
#endif
                    refCamera.RenderWithShader( worldFogMaterial.shader, "" );
#if UNITY_POST_PROCESSING_STACK_V2
                    postFX.enabled = _settings.usePostFX;
#endif
                }
            ...
            QualitySettings.shadows = tempShadows;
